...
try {for(const viewport of [{width:1440,height:900},{width:3840,height:2160},{width:390,height:844},{width:844,height:390}]) {
 ...
 for(const cursor of [0,TEACHING.findIndex(s=>s.type==='recruit'),TEACHING.findIndex(s=>s.type==='defend')]) {
  ...
  if(game.pending){await expect(page.locator('.c-target-arcs')).toBeVisible();const buttons=await page.locator('.c-response-actions').boundingBox();assert.ok(buttons);assert.ok(panel.y+panel.height<=buttons.y||panel.y>=buttons.y+buttons.height||panel.x+panel.width<=buttons.x||panel.x>=buttons.x+buttons.width,'Guide does not cover response buttons');}
  ...
  const labels=await page.locator('.core-played-label,.core-table-office,.core-table-caption,.core-table-seat:visible').evaluateAll(es=>es.map(e=>{const r=e.getBoundingClientRect();return{x:r.x,y:r.y,right:r.right,bottom:r.bottom,text:e.textContent};}));
