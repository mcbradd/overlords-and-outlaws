...
try{
  ...
  await stage('primary-hover-focus',{width:390,height:844},async(page,capture,row)=>{
    ...
    await page.keyboard.press('Enter');await clickReachable(page.locator('[data-do="teach"]'));await expect(page.locator('.c-game')).toBeVisible();await clickReachable(page.locator('[data-do="select"][data-card="alba-2"]'));const commit=page.locator('[data-do="choice"]');await commit.hover();row.facts.actionHover=await contrast(commit);await capture('action-hover');
  });
  ...
  await stage('keyboard-first-move',{width:390,height:844},async(page,capture)=>{
    await load(page);for(const selector of ['[data-do="intro"]','[data-do="teach"]','[data-do="select"][data-card="alba-2"]','[data-do="choice"]']){await expect(page.locator(selector)).toBeAttached();await tabTo(page,selector);await capture(`focus-${selector.match(/data-do="([^"]+)/)?.[1]}`);await page.keyboard.press('Enter');}
  ...
  await stage('touch-first-move',{width:390,height:844},async(page,capture)=>{
    await load(page);for(const selector of ['[data-do="intro"]','[data-do="teach"]','[data-do="select"][data-card="alba-2"]','[data-do="choice"]']){await geometry(page.locator(selector));await page.locator(selector).tap();}
