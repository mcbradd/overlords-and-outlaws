...
try{
  for(const dynasty of ['alba','plantagenet','tudor','habsburg'])await stage(`dynasty-${dynasty}`,{width:1440,height:900},async(page,capture)=>{
    await localStart(page,dynasty);await expect(page.locator('.c-hand .c-card-pick')).toHaveCount(2);const founder=CARDS.find(card=>card.dynasty===dynasty&&card.founder)!;await expect(page.locator(`[data-table-card="${founder.id}"]`)).toBeAttached();await capture('selected-native-family');
  });
