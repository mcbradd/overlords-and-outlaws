...
> **Player correction, 18 September 2026:** Claim names an heir without a separate supporter. Keep the existing ruler and heir in Court throughout the entire next round to win; the heir succeeds only when that full round ends. Removing either breaks the claim. A foreign heir still needs its actual marriage link, following the existing foreign-entry rule. Declaration is reversible until the player confirms all three chosen Nobles with Declare. These rules supersede the older supporter and early-inheritance language below.
