...
- **State truth:** all live views show current health. Damaged cards show current/maximum; counters still represent physical damage. Live cost badges account for the controlling House; inspection names a discount relative to printed cost. Reference/archive cards retain base values. The canonical data still defines printable components.
