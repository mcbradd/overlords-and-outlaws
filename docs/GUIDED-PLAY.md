...
> **18 September correction:** Tutorial UI is a non-modal layer over the exact ordinary-game geometry. Never resize the board/hand for teaching. Play scripted rival actions first, show their source-to-target arc and card movement, then explain the actual player response. No Watch-the-table gates. Declaration is reversible until Declare. Claim requires ruler plus heir, no supporter; protect both through the complete next round. Teach that objective before claiming, show hold progress, and celebrate the completed succession. Yellow attention glows beneath the whole card; green remains selected.

> **Retreat and Recall language:** Recall returns your own Court Noble to hand and uses your turn. Challenge targets a rival. Buttons carry plain-English hover hints. The legal tutorial now demonstrates an unanswerable rank-10 challenge against Alexander III before claiming the Crown; Retreat is taught at the actual response button, with a named explanation. Normal play also explains missing or insufficient defenders directly beside Retreat.

> **Draft readiness and sorting:** Computer players privately preselect their packets while the human thinks; PASS starts the simultaneous exchange without per-card AI pauses. Hands default to Dynasty then Rank. The bottom Sorted by DYNASTY / Sorted by RANK toggle changes display order only and retains selected cards.

> **Current player correction, 17 September 2026:** Remove the top interaction zone. Raise the opaque hand fan, tuck right edges underneath the following card, and center PASS beneath it. Rank and Dynasty occupy a large full-bleed top-left heraldic corner. Green marks selection; yellow invitations on buttons pulse subtly, respecting reduced motion. The tutorial uses an independent nonmodal overlay with no dimming backdrop or layout changes, waits for table movement, and says Continue. Challenges replaces Recalls in all player-facing language. Hovering a legal defense card exposes a one-click Defend; touch selects the card and then Defend commits. Direct dragging anywhere on the table also commits a single concrete legal choice. Never require Defend followed by dragging. Retreat sits within thumb reach beneath the hand. Multiple legal choices still require choosing the intended action/target.
